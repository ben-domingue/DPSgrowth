teacher_MedianGP<-function(stud, #this is meant to be applied to a list where data is split by teacher.
                         N.stud=15,
                         bootstrap.N=500,
                         wt=NULL,
  ...
  median(stud$GrowthPercentile,wt)->mgp
  switch(CI.type,
         "confidence interval"={
           ...
           for (i in 1:bootstrap.N) {
             ...
             tmp[index,]->tmp.boot
             if (all(wt==1)) median(tmp.boot$GrowthPercentile)->boot[i] else stop("weight problem") #weighted.mean(tmp.boot$GrowthPercentile,wt=wt)->boot[i]
           }
